...
const determineTabFromTransactionRecord = (tx: TransactionListItem): Tab => {
  const code = normalizeTransactionNature((tx as any)?.transaction_nature ?? null);
...
const extractCashbackInfoFromTransaction = (
  ...
  const percentValue = Number((tx as any)?.cashback_percent ?? 0) || 0;
  const amountValue = Number((tx as any)?.cashback_amount ?? 0) || 0;
...
export default function TransactionForm({
  ...
  const defaultAmount = editingTransaction ? String(Math.round(editingTransaction.amount)) : persistedState?.amount ?? "";
  ...
  const defaultDebtMode: DebtMode =
    editingTransaction && defaultTabValue === "debt"
      ? ((editingTransaction as any)?.debtMode === "collect" ? "collect" : "lend")
  ...
  const defaultDebtTag = (editingTransaction as any)?.debtTag ?? persistedState?.debtTag ?? fallbackDebtTag;
  const defaultDebtCycleTag = (editingTransaction as any)?.debtCycleTag ?? persistedState?.debtCycleTag ?? fallbackDebtTag;
  ...
  const mapNatureToTabMemo = useCallback(
    (nature: TransactionNatureCode | null) => mapNatureToTab(nature),
    []
  );
